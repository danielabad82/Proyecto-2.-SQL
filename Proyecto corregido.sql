...
from film f 
where "language_id" is not null and "original_language_id" is not null;
...
where date(r."rental_date") >(
	select date(r."rental_date") as "Fecha_alquiler"
	from film f 
	inner join inventory i 
		on f."film_id"=i."film_id"
	inner join rental r 
...
--Lo hice de dos maneras diferentes, pero vi que de la segunda manera era más rápido y eficiente.
...
from Dias_alquilada
inner join inventory i 
	on Dias_alquilada."inventory_id"=i."inventory_id"
inner join film f 
	on i."film_id"=f."film_id"
where extract (day from "Dias_alquiler")>8
order by f."title";
